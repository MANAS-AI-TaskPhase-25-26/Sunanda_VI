...
|---	feature\_matching.py

|---	clustering/Grouping.py
...
What you need to do: Extract roads from the stitched map without any pretrained model. Roads in aerial maps typically have:
...
Convert to LAB or HSV color space — roads are easier to isolate there
...
Morphological operations: erosion, dilation, opening, closing — understand them geometrically, not just as API calls
...
Honest caveat: Pure classical road segmentation is hard. Your results will depend heavily on how the images look. Plan for iteration on the color thresholds.
...
2\.  Create SIFT detector and Detect Keypoints
...
3\. Draw Keypoints and Visualize the Image



img\_with\_keypoints = cv2.drawKeypoints(

&#x20;   img, keypoints, None, flags=cv2.DRAW\_MATCHES\_FLAGS\_DRAW\_RICH\_KEYPOINTS
...
plt.imshow(cv2.cvtColor(img\_with\_keypoints, cv2.COLOR\_BGR2RGB))
...
1\. Convert the training image to RGB

training\_image = cv2.cvtColor(image1, cv2.COLOR\_BGR2RGB)



2\. Convert the training image to gray scale

training\_gray = cv2.cvtColor(training\_image, cv2.COLOR\_RGB2GRAY)



3\. Display traning image and testing image

fx, plots = plt.subplots(1, 2, figsize=(20,10))



plots\[0].set\_title("Training Image")

plots\[0].imshow(training\_image)



4

surf = cv2.xfeatures2d.SURF\_create(800)



train\_keypoints, train\_descriptor = surf.detectAndCompute(training\_gray, None)

keypoints\_without\_size = np.copy(training\_image)

keypoints\_with\_size = np.copy(training\_image)

&#x20;

5

cv2.drawKeypoints(training\_image, train\_keypoints, keypoints\_without\_size, color = (0, 255, 0))



cv2.drawKeypoints(training\_image, train\_keypoints, keypoints\_with\_size, flags = cv2.DRAW\_MATCHES\_FLAGS\_DRAW\_RICH\_KEYPOINTS)



6 Display image with and without keypoints size

fx, plots = plt.subplots(1, 2, figsize=(20,10))



plots\[0].set\_title("Train keypoints With Size")

plots\[0].imshow(keypoints\_with\_size, cmap='gray')



plots\[1].set\_title("Train keypoints Without Size")

plots\[1].imshow(keypoints\_without\_size, cmap='gray')



7 Print the number of keypoints detected in the training image

print("Number of Keypoints Detected In The Training Image: ", len(train\_keypoints))



8 Print the number of keypoints detected in the query image

print("Number of Keypoints Detected In The Query Image: ", len(test\_keypoints)
...
train\_img = cv2.imread('/path')

train\_img\_bw = cv2.cvtColor(train\_img, cv2.COLOR\_BGR2GRAY)



2\. Detecting Keypoints and Finding Descriptors



orb = cv2.ORB\_create()

trainKeypoints, trainDescriptors = orb.detectAndCompute(train\_img\_bw, None)



3\. Matching the Descriptors
...
final\_img = cv2.drawMatches(query\_img, queryKeypoints, 

&#x20;                            train\_img, trainKeypoints, matches\[:20], None)

final\_img = cv2.resize(final\_img, (1000, 650))



plt.figure(figsize=(10, 6))

plt.imshow(cv2.cvtColor(final\_img, cv2.COLOR\_BGR2RGB)) 
...
len(members)} fragments → {\[frag\_files\[m] for m in members]}")
...
Scale Space: Instead of reducing the image size (image pyramid) like SIFT, SURF increases the size of the box filters (e.g., 
...
\## just to TRY SURF (not in final pipeline) 



def extract\_features\_SURF(img) :

&#x20;   img\_bgr = cv2.cvtColor(img, cv2.COLOR\_BGR2RGB)

&#x20;   img\_gray = cv2.cvtColor(img, cv2.COLOR\_RGB2GRAY)

&#x20;   surf  = cv2.xfeatures2d.SURF\_create(800) #800

&#x20;   keypoints, descriptor = surf.detectAndCompute(img\_gray, None)

&#x20;   keypoints\_without\_size = np.copy(img\_gray)

&#x20;   keypoints\_with\_size = np.copy(img\_gray)

&#x20;   cv2.drawKeypoints(img\_gray, keypoints, keypoints\_without\_size, color = (0, 255, 0))

&#x20;   cv2.drawKeypoints(img\_gray, keypoints, keypoints\_with\_size, flags = cv2.DRAW\_MATCHES\_FLAGS\_DRAW\_RICH\_KEYPOINTS)
...
def show\_keypoints\_SURF(out\_dir, num=6):

&#x20;   files = \[f for f in os.listdir(out\_dir) if f.endswith(".jpg")]

&#x20;   for i in range(min(num, len(files))):

&#x20;       img\_path = os.path.join(out\_dir, files\[i])

&#x20;       frag = cv2.imread(img\_path)

&#x20;       kp, desc = extract\_features\_SURF(frag)



out\_dir = "DeepGlobe\_Dataset/fragments"

show\_keypoints\_SURF(out\_dir)
